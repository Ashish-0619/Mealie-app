...
pipeline {
    ...
    // Define global environment variables for the pipeline here
    environment {
        ...
        // Global variables for Docker and SonarQube URLs/credentials
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    ...
                    // withCredentials([usernamePassword(credentialsId: env.DOCKER_REGISTRY_CREDS_ID, passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME')]) {
                    //     sh "echo $DOCKER_PASSWORD | docker login -u $DOCKER_USERNAME --password-stdin ${env.DOCKER_REGISTRY}"
                    //     sh "docker tag ${env.DOCKER_IMAGE_NAME}:latest ${env.DOCKER_REGISTRY}/${env.DOCKER_IMAGE_NAME}:latest"
                    //     sh "docker push ${env.DOCKER_REGISTRY}/${env.DOCKER_IMAGE_NAME}:latest"
        ...
        stage('Container Scan - Trivy') {
            ...
            steps {
                script {
                    echo "Running Trivy scan on Docker image: ${env.DOCKER_IMAGE_NAME}:latest"
                    // Install Trivy (if not pre-installed on agent) or use a Trivy Docker image
                    // For example: docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image ${env.DOCKER_IMAGE_NAME}:latest
                    // For simplicity, assuming Trivy is installed on the local Jenkins agent.
                    def trivyCommand = "trivy image --format json -o trivy_report.json ${env.DOCKER_IMAGE_NAME}:latest"
                    try {
                        sh trivyCommand
                        ...
                        // sh "trivy image --exit-code 1 --severity CRITICAL,HIGH ${env.DOCKER_IMAGE_NAME}:latest"
                    } catch (Exception e) {
        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                withSonarQubeEnv(credentialsId: env.SONAR_QUBE_TOKEN_ID, installationName: 'SonarQube') { // 'SonarQube' is the name configured in Jenkins
                    ...
                    sh """
                        sonar-scanner \\
                            ...
                            -Dsonar.login=${env.SONAR_PROJECT_KEY}
                    """
